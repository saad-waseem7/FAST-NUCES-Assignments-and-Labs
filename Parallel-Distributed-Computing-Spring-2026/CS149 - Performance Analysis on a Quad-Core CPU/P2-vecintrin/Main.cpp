...
{
  // constant vectors
  __cs149_vec_int vecZeroI = _cs149_vset_int(0);
  __cs149_vec_int vecOneI = _cs149_vset_int(1);
  __cs149_vec_float vecOneF = _cs149_vset_float(1.f);
  __cs149_vec_float vecClamp = _cs149_vset_float(9.999999f);

  for (int base = 0; base < N; base += VECTOR_WIDTH)
  {
    ...
    _cs149_vload_int(y, exponents + base, maskActive);
    _cs149_vmove_float(result, vecOneF, maskActive); // initialize result=1

    __cs149_mask maskWork; // find lanes needing work (y>0)
    _cs149_vgt_int(maskWork, y, vecZeroI, maskActive);

    while (_cs149_cntbits(maskWork) > 0) // iterative exponentiation
    {
      _cs149_vmult_float(result, result, x, maskWork);
      _cs149_vsub_int(y, y, vecOneI, maskWork);
      _cs149_vgt_int(maskWork, y, vecZeroI, maskActive); // recompute active work mask
    }

    __cs149_mask maskClamp; // clamp to max value
    _cs149_vgt_float(maskClamp, result, vecClamp, maskActive);
    _cs149_vmove_float(result, vecClamp, maskClamp);

    _cs149_vstore_float(output + base, result, maskActive); // store result
  }
}
